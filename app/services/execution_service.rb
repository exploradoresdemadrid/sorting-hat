class ExecutionService
  ...
  def run
    vns = VNS::VNS.new(people.pluck(:id), people.pluck(:group), sessions.pluck(:id), preferences, event.max_capacity, event.group_duplication_factor) do |progress, target_function, solution|
      execution.update(progress: progress, target_function: target_function - people.count)
